...
def display_some_examples(examples, labels):
    ...
    for i in range(25):
        ...
        plt.subplot(5, 5, i + 1)
        plt.title(str(label))
        plt.tight_layout() # automatically add more space
...
def split_data(path_to_data, path_to_save_train, path_to_save_val, split_size=0.1):
    ...
    for folder in folders:
        ...
        images_paths = glob.glob(os.path.join(full_path, '*.png')) # look inside a folder and load all of the files exist depends on chosen extension
...
def create_generators(batch_size, train_data_path, val_data_path, test_data_path):
    train_preprocessor = ImageDataGenerator(
        rescale = 1 / 255., # rescale image
        rotation_range=10,
        width_shift_range=0.1, #shift n from left to right and from right to left
    ...
    # tensorflow func automatically find out the labels based on directory name
    train_generator = train_preprocessor.flow_from_directory(
        train_data_path,
        class_mode="categorical",
        target_size=(60, 60), # resize image to (60, 60)
        color_mode="rgb",
        shuffle=True,
